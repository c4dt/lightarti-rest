...
#[no_mangle]
pub unsafe extern "C" fn call_google() -> CFStringRef {
    match blocking_google_ch(){
        Ok(s) => to_cf_str(format!("Google.ch says: {}", s)),
        Err(e) => to_cf_str(format!("Error while calling google: {}", e)),
    }
}
